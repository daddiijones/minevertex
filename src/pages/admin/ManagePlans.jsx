  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 20 }}>
        <h3 style={{ fontWeight: 700 }}>Mining Plans ({plans.length})</h3>
        <button className="btn btn-primary" onClick={() => openEdit(null)}><Plus size={16} /> Add Plan</button>
      </div>
      <div className="table-container"><table className="data-table">
        <thead><tr><th>Plan</th><th>Hash Rate</th><th>Daily ROI</th><th>Total ROI</th><th>Duration</th><th>Min-Max</th><th>Status</th><th>Actions</th></tr></thead>
        <tbody>
          {plans.map(p => (
      ...
      {editing !== null && (
        <div className="modal-overlay" onClick={() => setEditing(null)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3>{editing === 'new' ? 'Create Mining Plan' : 'Edit Plan'}</h3>
            {[{ k: 'name', l: 'Plan Name', t: 'text' }, { k: 'hashRate', l: 'Hash Rate', t: 'text' }, { k: 'dailyROI', l: 'Daily ROI (%)', t: 'number' }, { k: 'totalROI', l: 'Total ROI (%)', t: 'number' }, { k: 'durationDays', l: 'Duration (Days)', t: 'number' }, { k: 'minDeposit', l: 'Min Deposit ($)', t: 'number' }, { k: 'maxDeposit', l: 'Max Deposit ($)', t: 'number' }].map(f => (
            ...
            <div className="form-group"><label className="form-label">Tier</label><select className="form-input" value={form.tier || 'starter'} onChange={e => setForm({ ...form, tier: e.target.value })}><option value="starter">Starter</option><option value="professional">Professional</option><option value="enterprise">Enterprise</option><option value="vip">VIP</option></select></div>
