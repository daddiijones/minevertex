  ...
  return (
    <div>
      <div className="stats-grid" style={{ marginBottom: 24 }}>
        ...
        <div className="stat-card"><div className="stat-card-header"><div><div className="stat-value">${data.totalBonus.toFixed(2)}</div><div className="stat-label">Total Bonus Earned</div></div><div className="stat-icon green"><DollarSign size={20} /></div></div></div>
        <div className="stat-card"><div className="stat-card-header"><div><div className="stat-value">5%</div><div className="stat-label">Commission Rate</div></div><div className="stat-icon purple"><Gift size={20} /></div></div></div>
      </div>
      ...
      <div className="card" style={{ marginBottom: 24 }}>
        ...
        <p style={{ color: 'var(--text-secondary)', fontSize: '0.88rem', marginBottom: 16 }}>Share your link and earn <strong style={{ color: 'var(--success)' }}>5%</strong> commission on every deposit your referrals make.</p>
      ...
      {data.referrals.length === 0 ? <div className="card empty-state"><p>No referrals yet. Share your link to start earning!</p></div> : (
        <div className="table-container"><table className="data-table"><thead><tr><th>User</th><th>Email</th><th>Joined</th><th>Bonus Earned</th></tr></thead><tbody>
          {data.referrals.map(r => (<tr key={r.id}><td style={{ fontWeight: 600, color: 'var(--text-primary)' }}>{r.referred?.fullName}</td><td>{r.referred?.email?.replace(/(.{3}).*(@.*)/, '$1***$2')}</td><td>{new Date(r.createdAt).toLocaleDateString()}</td><td style={{ fontWeight: 700, color: 'var(--success)' }}>${r.bonusEarned.toFixed(2)}</td></tr>))}
        </tbody></table></div>
      )}
    </div>
  )
