...
import { Cpu, DollarSign, TrendingUp, Zap, Activity } from 'lucide-react'
  ...
  return (
    <div>
      <div className="stats-grid" style={{ marginBottom: 24 }}>
        <div className="stat-card"><div className="stat-card-header"><div><div className="stat-value">{minings.filter(m => m.status === 'ACTIVE').length}</div><div className="stat-label">Active Minings</div></div><div className="stat-icon cyan"><Cpu size={20} /></div></div></div>
        <div className="stat-card"><div className="stat-card-header"><div><div className="stat-value">${fmt(totalActive)}</div><div className="stat-label">Total Invested</div></div><div className="stat-icon purple"><DollarSign size={20} /></div></div></div>
        <div className="stat-card"><div className="stat-card-header"><div><div className="stat-value" style={{ color: 'var(--success)' }}>${fmt(totalEarnedAll)}</div><div className="stat-label">Total Earned</div></div><div className="stat-icon green"><TrendingUp size={20} /></div></div></div>
      </div>
      ...
      {filtered.length === 0 ? <div className="card empty-state"><p>No {tab} minings.</p></div> : (
            ...
            return (
              <div key={m.id} className="card card-glow" style={{ position: 'relative', overflow: 'hidden' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, margin: '20px 0' }}>
                  {[
                    { l: 'Invested', v: `$${fmt(m.investedAmount)}` },
                    { l: 'Daily', v: `$${fmt(m.dailyEarning)}`, c: 'var(--success)' },
                    { l: 'Earned', v: `$${isActive ? fmt4(currentEarned) : fmt(currentEarned)}`, c: 'var(--accent-cyan)' },
